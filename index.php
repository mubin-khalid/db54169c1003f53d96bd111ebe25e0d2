<?php
	...
	class pdOnline {
		...
		public function setCookie(){
			$page = Spider::spider($this->header, false, $this->mainURI);
			$this->post = $this->getViewState($page).$this->toSend;
			Spider::spider($this->header, false, $this->mainURI, false, $this->post);
		...
		public function search($stTo = 1, $stFrom = 1, $street = '', $suburb = ''){
			...
			$view = $this->getViewState(Spider::spider($this->header, $this->referer, $this->mainURI.'?page=search'));
			...
			$page = Spider::spider($this->header, $this->referer, $this->mainURI.'?page=search', false, $this->post);
			$page = str_get_html($page);
			$links = array();
			...
			foreach($links as $link){
				...
				$page = Spider::spider($this->header, false, str_replace("&amp;", "&", $link));
	...
	$data = ($scrapper->parseFile("input.csv"));
	$scrapper->setCookie();
